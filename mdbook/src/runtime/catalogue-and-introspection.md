...
snapshot and still obey relation read authority. `MentionedFact` and
`ExtensionalMentionedFact` currently expose the same stored-fact rows; the latter names the
extensional-only contract explicitly for code that must exclude rule-derived results.
